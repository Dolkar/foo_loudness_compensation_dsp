...
These are then subtracted from each other to produce a frequency-dependent gain that forms the basis of the compensation. The EQ is then applied to the audio signal in real-time
through a partitioned FFT convolution algorithm. The DSP adds no additional latency and requires little processing power.
